...
Run `pnpm report:persist:args` to print the generated persistence payloads for `reports.persistGenerated`. In a live Convex deployment, replace fixture external IDs with live Convex document IDs and run the mutation with authentication, for example `convex run reports:persistGenerated '<args>'`. Local validation does not perform the live write because `reports.persistGenerated` requires authenticated live Convex document IDs.
