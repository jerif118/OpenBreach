...
Current repository note: a TanStack Start starter scaffold, pnpm lockfile, and Convex generated client files now exist, but the DEFF-ACC product implementation is still pending. The next implementation pass should convert the starter routes and metadata into the municipal risk-map app, add real Convex schema/functions and Clerk auth config, and then add Mastra/TanStack AI report workflows.
