...
import { enrichScanEvidence, enrichScanEvidenceBatch, riskLevelForScore } from "../src/scanner/risk.ts";
import { rawScanEvidenceSchema, scanResultSchema } from "../src/shared/contracts.ts";
...
const criticalEvidence = rawScanEvidenceSchema.parse({
  ...
  tls: { valid: false, expiresAt: "2025-01-01T00:00:00.000Z", issuer: "Fixture CA" },
  cms: {
    name: "wordpress",
    version: "6.4",
    confidence: 0.8,
  ...
  adminExposure: [{ path: "/wp-login.php", method: "HEAD", reachable: true, httpStatus: 200 }],
  errors: [],
});
...
assert.equal(criticalResult.findings.some((finding) => finding.category === "known-vulnerability"), true);
assert.equal(criticalResult.findings.every((finding) => finding.evidence.length > 0), true);
assert.equal(criticalResult.findings.every((finding) => finding.remediationHint.length > 0), true);
...
const lowConfidenceCms = enrichScanEvidence({
  ...
  cms: { name: "wordpress", version: "6.4", confidence: 0.79, evidence: ["generator:WordPress 6.4"] },
  adminExposure: [{ path: "/wp-login.php", method: "HEAD", reachable: false, httpStatus: 404 }],
});
assert.equal(lowConfidenceCms.findings.some((finding) => finding.category === "known-vulnerability"), false);

const fixturePath = "data/scans/latest.enriched-scan-results.json";
const fixtureResults = scanResultSchema.array().parse(JSON.parse(await readFile(fixturePath, "utf8")));
const levels = new Set(fixtureResults.map((result) => result.riskLevel));
assert.deepEqual([...levels].sort(), ["critical", "high", "low", "medium"]);

const rawResults = rawScanEvidenceSchema.array().parse(JSON.parse(await readFile("data/scans/latest.scan-results.json", "utf8")));
const persistenceArgs = toEnrichedScanPersistenceArgs(enrichScanEvidenceBatch(rawResults));
