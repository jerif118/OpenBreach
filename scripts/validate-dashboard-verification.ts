...
const hookSource = readFileSync("src/features/dashboard/use-dashboard-municipalities.ts", "utf8");
...
for (const snippet of requiredRouteSnippets) {
  if (!routeSource.includes(snippet)) {
    throw new Error(`Dashboard route is missing live query error-path fallback evidence: ${snippet}`);
  }
}

if (!hookSource.includes("useQuery(api.municipalities.list, {})")) {
  throw new Error("Dashboard live path must continue to use the real api.municipalities.list query.");
}

if (routeSource.includes("toDashboardMunicipalityState(new Error")) {
  throw new Error("Task 6 verification must not rely on adapter-only synthetic errors.");
}
