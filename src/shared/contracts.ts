...
export const scopeRateLimitSchema = z.object({
  requestsPerMinute: z.number().int().positive(),
});

export type ScopeRateLimit = z.infer<typeof scopeRateLimitSchema>;

export const scopeApprovalRequirementSchema = z.object({
  required: z.boolean(),
  approverName: nonEmptyStringSchema.optional(),
  rationale: nonEmptyStringSchema,
});

export type ScopeApprovalRequirement = z.infer<
  typeof scopeApprovalRequirementSchema
>;

export const scopeTestWindowSchema = z
  .object({
...
export type ScopeTestWindow = z.infer<typeof scopeTestWindowSchema>;
...
export const targetProfileSchema = z
  ...
  .superRefine((value, ctx) => {
    if (
      value.classification === "public-sector" &&
...
export const authorizationScopeSchema = z
  .object({
    ...
    allowedAssets: z.array(nonEmptyStringSchema).min(1),
    deniedAssets: z.array(nonEmptyStringSchema).default([]),
    allowedValidationClasses: z.array(validationClassSchema).min(1),
    allowedActions: z.array(nonEmptyStringSchema).min(1),
    deniedActions: z.array(nonEmptyStringSchema).min(1),
    rateLimit: scopeRateLimitSchema,
    approvalRequirement: scopeApprovalRequirementSchema,
    testWindow: scopeTestWindowSchema.optional(),
    constraints: z.array(nonEmptyStringSchema).optional(),
    evidenceUrl: z.string().url().optional(),
  })
  .superRefine((value, ctx) => {
    if (
      value.scopeType === "time-bound" &&
      (!value.expiresAt ||
        new Date(value.expiresAt) <= new Date(value.grantedAt))
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (
      value.allowedValidationClasses.includes("controlled_validation") &&
      !value.approvalRequirement.required
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "controlled_validation scopes must require an approval gate",
        path: ["approvalRequirement", "required"],
      });
    }

    if (
      value.approvalRequirement.required &&
      !value.approvalRequirement.approverName
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "approverName is required when approval is required",
        path: ["approvalRequirement", "approverName"],
      });
    }
    ...
    if (
      value.testWindow &&
      value.expiresAt &&
      value.testWindow.endAt !== value.expiresAt
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "testWindow.endAt must match expiresAt when expiresAt is set",
        path: ["testWindow", "endAt"],
      });
    }
  });
...
export const workflowRunSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      (value.status === "halted" || value.status === "rejected") &&
      (!value.abortedAt || !value.abortedReason)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (value.status === "rejected" && value.phases) {
      const lastPhase = value.phases[value.phases.length - 1];
      if (!lastPhase?.rejectionReason) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "latest phase must include rejectionReason",
          path: ["phases", value.phases.length - 1, "rejectionReason"],
        });
      }
    }
  });
...
export const passiveScanEvidenceSchema = z
  ...
  .superRefine((value, ctx) => {
    if (value.reachable && value.httpStatus === undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (value.tls?.expiresAt && new Date(value.tls.expiresAt) <= new Date()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const technologyFingerprintSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    const hasVersionConfidence = value.versionConfidence !== undefined;
...
export const vulnerabilityHypothesisSchema = z
  ...
  .superRefine((value, ctx) => {
    const rejectionReason =
      ...
      "rejectionReason" in value.metadata;

    if (value.status === "rejected" && !rejectionReason) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const testPlanSchema = z
  ...
  .superRefine((value, ctx) => {
    const requiresApproval = ["approved", "executing", "completed"].includes(
      value.status,
    );
    const rejectionReason =
      value.metadata &&
    ...
    if (requiresApproval && (!value.approver || !value.approvedAt)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
...
export const approvalGateSchema = z
  ...
  .superRefine((value, ctx) => {
    ...
    if (
      value.status === "bypassed" &&
      (!value.bypassJustification || value.bypassJustification.length < 10)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const findingSchema = z
  ...
  .superRefine((value, ctx) => {
    if (value.status === "confirmed" && !value.validationResultId) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (value.severity === "critical" && value.reportReady === undefined) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
    ...
    if (value.affectedAssets?.some((asset) => asset.trim().length === 0)) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const validationResultSchema = z
  ...
  .superRefine((value, ctx) => {
    const blockedReason =
      value.metadata &&
      typeof value.metadata === "object" &&
      "blockedReason" in value.metadata;

    if (
      value.status === "failed" &&
      (!value.findings || value.findings.length === 0)
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
...
export const auditEventSchema = z.object({
  ...
  ipAddress: z
    .string()
    .optional()
    .refine((value) => !value || /^(?:\d{1,3}\.){3}\d{1,3}$/.test(value), {
...
export const reportArtifactSchema = z
  ...
  .superRefine((value, ctx) => {
    const hasError =
      value.metadata &&
      typeof value.metadata === "object" &&
      "error" in value.metadata;

    if (
      value.status === "completed" &&
      (!value.pdf ||
        !/^data\/reports\/[A-Za-z0-9._-]+\.pdf$/.test(value.pdf.storagePath))
    ) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message:
    ...
    if (value.status === "failed" && !hasError) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
