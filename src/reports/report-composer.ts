...
const plainLanguageDictionary: Array<[RegExp, string]> = [
  [
    /\bHTTP Strict Transport Security\b/gi,
    "browser rule that forces secure connections",
  ],
  [
    /\bContent Security Policy\b/gi,
    "browser rule that limits what the site can load",
  ],
  [
    /\bX-Content-Type-Options\b/gi,
    "browser setting that blocks unsafe file-type guessing",
  ],
...
function adaptFinding(
  ...
  return {
    ...
    evidenceSummary: toSentence(
      toAudienceText(finding.evidenceSummary, variant),
    ),
    remediationHint: toSentence(
      toAudienceText(finding.remediationHint, variant),
    ),
    remediationSteps: finding.remediationSteps.map((step) =>
      toSentence(toAudienceText(step, variant)),
    ),
    verificationSteps: finding.verificationSteps.map((step) =>
      toSentence(toAudienceText(step, variant)),
    ),
  };
}
...
function buildFindingsOverview(
  input: NormalizedReportInput,
  variant: ReportAudience,
) {
  ...
  const bullets = [
    ...
    ...topTitles.map((title) =>
      variant === "friendly"
        ? `Top issue: ${translatePlainLanguage(title)}`
        : `Top finding: ${title}`,
    ),
  ];
    ...
    return {
      ...
      bullets:
        bullets.length > 0
          ? bullets
          : [
              "No reportable findings were supplied in the current input payload.",
            ],
    };
  }
  ...
  return {
    ...
    bullets:
      bullets.length > 0
        ? bullets.map((entry) => translatePlainLanguage(entry))
...
function buildReport(
  ...
  const findings = input.findings.map((finding) =>
    adaptFinding(finding, variant),
  );
  const findingsOverview = buildFindingsOverview(input, variant);
  const scopeTitle =
    variant === "technical" ? "Scope" : "What this report covers";
  const authorizationTitle =
    variant === "technical" ? "Authorization" : "What was allowed";
  const methodologyTitle =
    variant === "technical" ? "Methodology" : "How this was reviewed";
  const skippedTitle =
    variant === "technical"
      ? "Skipped and denied tests"
      : "What was not tested";
  const validationTitle =
    variant === "technical" ? "Validation status" : "What was confirmed";
  const limitationsTitle =
    variant === "technical" ? "Limitations" : "Important limits";
  ...
  return remediationReportSchema.parse({
    ...
    sections: {
      scope: {
        ...
        bullets: input.scopeBullets.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      authorization: {
        ...buildSectionNarrative(
          authorizationTitle,
          input.authorizationNarrative,
          variant,
        ),
        bullets: input.authorizationBullets.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      methodology: {
        ...buildSectionNarrative(
          methodologyTitle,
          input.methodologyNarrative,
          variant,
        ),
        bullets: input.methodologyBullets.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      findingsOverview: {
      ...
      skippedTests: {
        ...
        bullets: input.skippedTests.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      validationStatus: {
        ...
        bullets: input.validationStatus.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      limitations: {
        ...
        bullets: input.limitations.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      remediationChecklist: {
        ...
        bullets: input.remediationChecklist.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
      verificationGuidance: {
        ...
        bullets: input.verificationGuidance.map((bullet) =>
          toAudienceText(bullet, variant),
        ),
      },
    },
