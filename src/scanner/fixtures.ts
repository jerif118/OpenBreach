...
import { rawScanEvidenceSchema, scanResultSchema, type RawScanEvidence, type ScanResult } from "../shared/contracts.ts";
...
export function normalizeEnrichedScanFixtureResults(results: readonly unknown[]): ScanResult[] {
...
export async function exportEnrichedScanFixture(results: readonly RawScanEvidence[], outputPath: string): Promise<void> {
  const enrichedResults = normalizeEnrichedScanFixtureResults(enrichScanEvidenceBatch(results));
