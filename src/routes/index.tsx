...
import {
  getRiskDisplay,
  riskLegend,
} from "../features/dashboard/risk-display.ts";
...
function Home() {
  const source = getDashboardMunicipalitySource(
...
function DashboardRouteError({
  ...
  return (
    <DashboardFrame>
      <DashboardShell
        state={{ status: "error", source: "convex", items: [], message }}
      />
      <div className="pb-5">
        <button
          type="button"
          className="rounded-full border border-cyan-300/30 px-4 py-2 text-sm font-semibold text-cyan-100 transition hover:border-cyan-200 hover:bg-cyan-300/10 focus:ring-2 focus:ring-cyan-200 focus:outline-none"
          onClick={reset}
        >
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(34,211,238,0.18),_transparent_34rem),linear-gradient(135deg,_#020617_0%,_#0f172a_52%,_#111827_100%)] text-slate-100">
      <section className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-semibold tracking-[0.42em] text-cyan-300 uppercase">
              DEFF-ACC
            </p>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300 sm:text-base">
              Mexico-focused cybersecurity exposure for judges and municipal
              teams, with live-ready data regions for the risk map and priority
              queue.
            </p>
          </div>
...
function DashboardShell({ state }: { state: DashboardMunicipalityState }) {
  ...
  const filteredItems = getFilteredMunicipalities(
    readyItems,
    selectedRiskLevel,
    selectedState,
  );
  const filteredState: DashboardMunicipalityState =
    state.status === "ready" ? { ...state, items: filteredItems } : state;
  const rankedItems = getRankedMunicipalities(filteredItems);
  const stateOptions = [...new Set(readyItems.map((item) => item.state))].sort(
    (a, b) => a.localeCompare(b),
  );
  const hasActiveFilters =
    selectedRiskLevel !== "all" || selectedState !== "all";
      ...
      <aside className="grid gap-4 lg:grid-rows-[auto_1fr]">
        <section className="rounded-[2rem] border border-white/10 bg-white/10 p-4 shadow-xl shadow-black/20 backdrop-blur sm:p-5">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-300">
            Highest-risk municipalities stay defensively sorted and link to the
            detail route for demo-safe follow-up.
          </p>
...
function DashboardFilters({
  ...
  const filteredEmpty =
    readyCount > 0 && hasActiveFilters && matchedCount === 0;
    ...
    <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-slate-950/45 p-4">
      <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto] sm:items-end">
        ...
        >
          ...
          <select
            ...
            onChange={(event) =>
              onRiskLevelChange(event.target.value as RiskLevel | "all")
            }
          >
        ...
        <button
          type="button"
          className="rounded-full border border-cyan-300/30 px-4 py-2 text-sm font-semibold text-cyan-100 transition focus:ring-2 focus:ring-cyan-200 focus:outline-none enabled:hover:border-cyan-200 enabled:hover:bg-cyan-300/10 disabled:cursor-not-allowed disabled:opacity-45"
      ...
      {filteredEmpty ? (
        <div className="mt-3 rounded-2xl border border-amber-200/20 bg-amber-200/10 p-3 text-sm leading-6 text-amber-100">
          ...
          <button
            type="button"
            className="mt-2 font-semibold underline decoration-amber-100/50 underline-offset-4"
            onClick={onClearFilters}
          >
...
function SourceBadge({ state }: { state: DashboardMunicipalityState }) {
  const label =
    state.source === "convex" ? "Live Convex data" : "Demo mock data";
  const toneClassName =
    state.source === "convex"
    ...
    <ol className="mt-5 grid gap-3">
      {items.map((item, index) => {
        ...
        return (
          <li
            key={item.id}
            className="rounded-2xl border border-white/10 bg-slate-950/55 transition hover:border-cyan-300/40 hover:bg-slate-900/80"
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-right">
                ...
                <p
                  className={`text-sm font-medium ${legendItem.textClassName}`}
                >
                  {legendItem.label}
                </p>
              </div>
            ...
            <Link
              to="/municipalities/$id"
              params={{ id: item.id }}
              className="mt-4 inline-flex rounded-full border border-cyan-300/30 px-3 py-1.5 text-sm font-semibold text-cyan-100 transition hover:border-cyan-200 hover:bg-cyan-300/10 focus:ring-2 focus:ring-cyan-200 focus:outline-none"
            >
              Open detail
            </Link>
          </li>
    ...
    <section className="rounded-[2rem] border border-white/10 bg-slate-900/85 p-4 shadow-xl shadow-black/20 sm:p-5">
      ...
      <div className="mt-5 grid gap-3">
        {riskLegend.map((item) => (
          ...
          >
            <span
              className={`mt-1 h-4 w-4 shrink-0 rounded-full shadow-lg ${item.swatchClassName}`}
              aria-hidden="true"
            />
            <div>
              <p className={`font-semibold ${item.textClassName}`}>
                {item.label}
              </p>
...
function getStatePanelCopy(state: DashboardMunicipalityState) {
  switch (state.status) {
    ...
    case "error":
      return (
        state.message ||
        "The shell stays visible so the source issue can be diagnosed."
      );
    case "ready":
